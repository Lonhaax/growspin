--=== MAIN SETTINGS ===--
Target_Path = "C:\\Mac\\Home\\Desktop\\depo trade\\httpserver\\bots\\"..getBot().name..".json"
...
local function setWorlds()
    local bots = getBots()
    for index, process in sortedPairsNumeric(Process_Table) do
        local botak = bots[index]
        if botak then
            botlist[botak.name] = {
                Deposit_World = process.Deposit_World,
                Vault_World = process.Vault_World
            }
        else
            error("Cannot find bot with: getBots()[" .. index .. "]", 2)
        end
    end
end
...
local function tradeProcess(targetGrowID, targetAmount, mode)
    ...
    while not completed and reconnect(depoworld, "NONE", false) do
        ...
        for i = 1, #players do
            ...
            local playername = player.name:gsub("^Dr%.", "")
            if playername:lower() == targetGrowID:lower() then
